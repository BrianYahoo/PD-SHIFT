...
[[ -f "$SC_SIFT2" ]] || die "Missing SC output"
[[ -f "$SC_SIFT2_RADIAL4" ]] || die "Missing SC radial4 output"
...
# 将 4 个 trial 的 FC 矩阵取平均，并汇总每个 trial 的 QC。
...
# 复制 SC 结果到 final 根目录和 dwi 子目录。
cp -f "$SC_SIFT2" "${FINAL_DIR}/${SUBJECT_ID}_DTI_connectome_sift2.csv"
cp -f "$SC_COUNT" "${FINAL_DIR}/${SUBJECT_ID}_DTI_connectome_count.csv"
cp -f "$SC_SIFT2_RADIAL4" "${FINAL_DIR}/${SUBJECT_ID}_DTI_connectome_sift2_radial4.csv"
cp -f "$SC_COUNT_RADIAL4" "${FINAL_DIR}/${SUBJECT_ID}_DTI_connectome_count_radial4.csv"
cp -f "$SC_SIFT2" "${FINAL_DIR}/dwi/sc/"
cp -f "$SC_COUNT" "${FINAL_DIR}/dwi/sc/"
cp -f "$SC_SIFT2_RADIAL4" "${FINAL_DIR}/dwi/sc/"
cp -f "$SC_COUNT_RADIAL4" "${FINAL_DIR}/dwi/sc/"

"$PYTHON_BIN" "${UTILS_DIR}/split_sc_matrices.py" \
