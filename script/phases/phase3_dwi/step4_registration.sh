...
VIS_REG_DIR="${PHASE3_DWI_DIR}/visualization/registration"
VIS_REG_DONE="${VIS_REG_DIR}/split_overlay.done"

# 如果 DWI 空间下的配准和 5TT 结果已经存在，则直接跳过。
if [[ -f "${DWI_DIR}/t1_to_dwi.mat" && -f "${DWI_DIR}/atlas_in_dwi.nii.gz" && -f "${DWI_DIR}/aparc+aseg_int.nii.gz" && -f "${DWI_DIR}/aparc+aseg_dwi.nii.gz" && -f "${DWI_DIR}/5tt_dwi.mif" && -f "${DWI_DIR}/gmwmi_seed.mif" && ( "${DWI_5TT_FIX_HYBRID_SUBCGM:-1}" != "1" || -f "${DWI_DIR}/5tt_subcgm_fix.json" ) && -f "${VIS_REG_DONE}" ]]; then
...
if [[ "${DWI_5TT_FIX_HYBRID_SUBCGM:-1}" == "1" && ! -f "${DWI_DIR}/5tt_subcgm_fix.json" ]]; then
  ...
  "$PYTHON_BIN" "${UTILS_DIR}/repair_5tt_hybrid_subcgm.py" \
