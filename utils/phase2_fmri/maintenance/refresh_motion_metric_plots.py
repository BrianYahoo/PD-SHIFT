...
def iter_motion_dirs(root: Path):
    pattern = "*/*/visualization/phase2_fmri/*/motion"
    yield from root.glob(pattern)


def infer_motion_file(motion_dir: Path):
    trial_name = motion_dir.parent.name
    subject_work_root = motion_dir.parents[3]
    matches = sorted(subject_work_root.glob(f"derivatives/cns-pipeline/sub-*/phases/phase2_fmri/{trial_name}/func_mc.par"))
